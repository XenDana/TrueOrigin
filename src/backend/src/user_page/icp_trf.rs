use candid::{types::principal, CandidType, Nat, Principal};

use ic_cdk::{api::{self, management_canister::main::{CanisterId}}, update,query};
// use ic_cdk_macros::*;
...
async fn get_balance() -> Result<Nat, String> {
    let principal = api::caller();
    let account: Account = Account::from(principal);
    ic_cdk::println!("Getting balance for account {}", &account);

    let balance = ic_cdk::call::<(Account,), (NumTokens,)>(
        Principal::from_text("mxzaz-hqaaa-aaaar-qaada-cai")
            .expect("Could not decode the principal."),
        "icrc1_balance_of",
        (account,),
